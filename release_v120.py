#!/usr/bin/env python3
"""Create GitHub release v1.2.0 and upload assets."""
...
ASSETS = [
    ("BLACK_BOX_Mobile_v1.2.0.apk", "application/vnd.android.package-archive"),
    ("version.json", "application/json"),
]

BODY = (
    "- Мульти-поиск больше не блокирует экран: анализ работает в фоне, "
    "можно переходить в другие разделы.\n"
    "- Добавлена кнопка «Остановить анализ» — реальная остановка поиска.\n"
    "- При возврате в мульти-поиск видно текущий прогресс и результаты.\n"
    "- Прогресс анализа дублируется в уведомлении.\n"
    "- В меню (три точки) добавлен пункт «Аккаунт»: баланс и открытые позиции биржи в реальном времени.\n"
    "- Новости релизов теперь на русском."
)
...
def api_headers(token, host="api.github.com"):
    return {
        ...
        "Host": host,
        "User-Agent": "release-v120-script",
    }
