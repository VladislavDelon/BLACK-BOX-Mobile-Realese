#!/usr/bin/env python3
"""Create GitHub release v1.2.1 and upload assets."""
...
ASSETS = [
    ("BLACK_BOX_Mobile_v1.2.1.apk", "application/vnd.android.package-archive"),
    ("version.json", "application/json"),
]

BODY = (
    "- Анализ теперь на 100% совпадает с десктопом: свечи грузятся со спота Binance "
    "(api.binance.com), как в десктопной версии.\n"
    "- Глубина истории возвращена к 1000 свечей — как на десктопе.\n"
    "- find_peaks приведён к точному поведению scipy (плато считается пиком в середине).\n"
    "- Список монет остаётся фьючерсным — ровно как на десктопе."
)
...
def api_headers(token, host="api.github.com"):
    return {
        ...
        "Host": host,
        "User-Agent": "release-v121-script",
    }
