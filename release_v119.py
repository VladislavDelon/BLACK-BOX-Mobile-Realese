#!/usr/bin/env python3
"""Create GitHub release v1.1.9 and upload assets."""
...
ASSETS = [
    ("BLACK_BOX_Mobile_v1.1.9.apk", "application/vnd.android.package-archive"),
    ("version.json", "application/json"),
]

BODY = (
    "- Fix NameError in signal windows.\n"
    "- Pattern data source switched to Binance USD-M Futures (fapi) to match symbol list.\n"
    "- More informative error handling for missing symbols.\n"
    "- Chart and movement windows remain available after analysis."
)
...
def api_headers(token, host="api.github.com"):
    return {
        ...
        "Host": host,
        "User-Agent": "release-v119-script",
    }
...
def delete_existing_assets(token, release):
    """Удаляет ассеты с теми же именами, чтобы можно было перезалить."""
