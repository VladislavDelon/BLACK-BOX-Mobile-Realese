#!/usr/bin/env python3
"""Create GitHub release v1.2.3 and upload assets."""
...
ASSETS = [
    ("BLACK_BOX_Mobile_v1.2.3.apk", "application/vnd.android.package-archive"),
    ("version.json", "application/json"),
]

BODY = (
    "- Мульти-поиск показывает реальные данные как на десктопе: "
    "направление (LONG/SHORT/FLAT) по среднему прогнозу всех паттернов, "
    "сила сигнала «6/10» и процент движения — вместо «NEUTRAL» для слабых сигналов.\n"
    "- Одна строка на монету: «Загрузка...» заменяется результатом, список не дублируется.\n"
    "- Счётчик прогресса в формате десктопа: «Анализ 5/722».\n"
    "- Звуковой сигнал срабатывает по |прогноз| ≥ порога и для SHORT тоже."
)
...
def api_headers(token, host="api.github.com"):
    return {
        ...
        "Host": host,
        "User-Agent": "release-v123-script",
    }
